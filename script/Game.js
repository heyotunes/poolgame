...
/*This code controls how a pool game runs and keeps everything moving smoothly on the screen. It starts by checking the best way to keep the game running at 60 frames per second so that it feels smooth. It manages the game's setup, like loading images (sprites), setting the size of the game, and starting the main menu. It also initializes the game world where the pool balls and table are drawn, and prepares the AI if you're playing against the computer. The `mainLoop` function keeps updating the game by handling user inputs (like key presses), updating positions, and drawing everything over and over. This makes sure the game responds quickly and looks good while playing.*/
