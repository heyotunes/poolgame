...
/**This code controls the pool cue (stick) used to hit the white ball in a pool game. It tracks the position, rotation, power, and visibility of the stick. The `handleInput` function allows the player to adjust the stick's power using the keyboard, rotate it using the mouse, and shoot when the left mouse button is clicked. When the stick shoots, the power and direction are used to hit the white ball. The stick becomes invisible for a short time after shooting. The `reset` function moves the stick back to the white ball's position after each shot, preparing it for the next turn. */
