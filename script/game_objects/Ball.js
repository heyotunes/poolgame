...
/*This code defines the behavior of a pool game ball in various stages of the game. 
The Ball constructor initializes key properties like its position, velocity, color, and whether
 it's moving or in a hole. A getter and setter for the ball's color dynamically update its appearance
  based on pre-defined sprite images. The shoot method calculates the ball's velocity when shot with a 
  given power and angle. The update method continuously moves the ball, reduces its velocity due to friction, 
  and stops it if the speed becomes minimal. If the ball falls into a hole, it becomes invisible and stops
   moving. The handleCollision method handles collisions with the pool table's borders by reversing the ball's velocity,
    simulating a bounce. Additionally, the ball can be reset to its original position, or placed out of play (invisible) 
    when needed. This functionality simulates realistic ball movement, collision, and scoring in the pool game.*/
